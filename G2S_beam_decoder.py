...
def search(sess, model, vocab, batch, options, decode_mode='greedy'):
    ...
    word_t = batch.sent_inp[:,0] # <s> 上一步得到的输出
    ...
    for i in xrange(options.max_answer_len): # 一步步预测输出单词
        if decode_mode == "pointwise": word_t = batch.sent_inp[:,i] # teacher forcing
        feed_dict = {}
    ...
    generator_input_idx = np.stack(generator_input_idx, axis=1) # [batch_size, max_len] 多个矩阵合在一起
...
class Hypothesis(object):
    ...
    def idx_seq_to_string(self, passage, vocab, options):
        ...
        for i, idx in enumerate(self.tokens):
            ...
            if cur_word == 'UNK':  #用attention权值最大的词来代替
...
def run_beam_search(sess, model, vocab, batch, options):
    ...
    # (tokens, log_ps, attn, state, context_vector, coverage_vector=None)
    ...
    while steps < options.max_answer_len and len(results) < options.beam_size:
        ...
        cur_context_t_1 = np.stack(cur_context_t_1, axis=0) #变成[batch_size, encoder_dim]
        cur_coverage_t_1 = np.stack(cur_coverage_t_1, axis=0) #变成[batch_size, passage_len]
        cur_word_t = np.array(cur_word_t)
        # 组建batch的时候用hyps里的假设组建
        ...
        # state_t.c[i:i+1, :] 取出第i个hyps的new_state cell

        # Extend each hypothesis and collect them all in all_hyps
        if steps == 0: cur_size = 1 #记录当前有几个hyps
        all_hyps = []
        for i in xrange(cur_size):
            ...
            for j in xrange(options.beam_size): #每一个hyps构造beam_size个输出
...
if __name__ == '__main__':
    ...
    parser.add_argument('--graph_cate', type=str, default='rdf', help='graph category')
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Valid"):
            with tf.variable_scope("Model", reuse=False, initializer=initializer):
                valid_graph = ModelGraph(word_vocab=word_vocab, char_vocab=char_vocab, Edgelabel_vocab=edgelabel_vocab, POS_vocab = POS_vocab,
