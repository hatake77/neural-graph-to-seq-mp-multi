...
class CovCopyAttenGen:
    ...
    def one_step_decoder(self, state_t_1, context_t_1, coverage_t_1, word_t, encoder_states, encoder_features,
        ...
        x = linear([word_t, context_t_1], options.attention_vec_size, True) # 对于每个时间步t,decoder将当前输入的embedding et和前一次的上下文向量μt-1连接后输入到LSTM去更新隐藏状态。

        # Run the decoder RNN cell. cell_output = decoder state
        cell_output, state_t = self.cell(x, state_t_1) # LSTM
    ...
    def train_mode(self, vocab, encoder_dim, encoder_states, encoder_features, node_idxs, node_mask,
        ...
        decoder_inputs: [batch_size, max_dec_steps].  <s>+sentenceidx 上一步输出结果
        answer_batch: [batch_size, max_dec_steps]     sentenceidx+</s> 这一步参考结果
        '''
        options = self.options
        ...
        decoder_inputs = tf.unstack(decoder_inputs, axis=1) # max_enc_steps * [batch_size] 矩阵分解为max_enco_steps个
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            for i in range(options.max_answer_len): # 从头到尾预测句子
                ...
                if mode_gen == 'greedy':
                    ...
                    wordidx_t = answer_batch_unstack[i]  #teacher_forcing
                else:
    ...
    def calculate_encoder_features(self, encoder_states, encoder_dim):
        ...
        batch_size = input_shape[0]
        passage_len = input_shape[1]
        # 这里卷积操作的用意是给每个位置的单词做相同处理吗，目的其实只是为了转换维度？
        with variable_scope.variable_scope("attention_decoder"):
    ...
    def decode_mode(self, word_vocab, beam_size, state_t_1, context_t_1, coverage_t_1, word_t,
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            multinomial_prediction = tf.reshape(tf.multinomial(vocab_scores, 1),[-1]) # calculate multinomial 采样
