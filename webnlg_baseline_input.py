...
def delexicalisation(out_src, out_trg, category, properties_objects):
    ...
    """
    # delex_dict.json中存放着与category相关的稀疏词
    ...
    # replace all occurrences of objects by PROPERTY in input ？？？？ 为什么要把object替换为property？
    for pro, obj in sorted(properties_objects.items()):
        ...
        if obj_clean in delex_src:
            ...
            replcments[pro.upper()] = ' '.join(obj_clean.split())   # remove redundant spaces 记录下替换词？
