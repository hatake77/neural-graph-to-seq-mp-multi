...
class GraphEncoder(object):
    def __init__(self, word_vocab=None, edge_label_vocab=None, char_vocab=None, POS_vocab = None, is_training=True, options=None):
        assert options != None
        ...
        if options.with_char:
            ...
            # [batch_size, passage_nodes_size_max, passage_nodes_chars_size_max]
            self.passage_nodes_chars = tf.placeholder(tf.int32, [None, None, None])
        self.passage_nodes_POS_size = tf.placeholder(tf.int32, [None])
        self.passage_nodes_POS = tf.placeholder(tf.int32, [None, None])
        ...
        passage_nodes_POS_size_max = passage_nodes_size_max
        ...
        self.POS_embedding = tf.get_variable("POS_embedding", trainable = False, 
                                             initializer=tf.constant(POS_vocab.word_vecs), dtype=tf.float32)

        word_dim = word_vocab.word_dim
        edge_dim = edge_label_vocab.word_dim
        POS_dim = POS_vocab.word_dim
        ...
        passage_nodes_POS_representation = tf.nn.embedding_lookup(self.POS_embedding, self.passage_nodes_POS)
        passage_nodes_POS_representation = tf.reshape(passage_nodes_POS_representation,
                                                      shape=[batch_size, passage_nodes_POS_size_max, POS_dim])
		
        if options.with_char:
            input_dim = word_dim + options.char_lstm_dim + POS_dim
            passage_node_representation = tf.concat([passage_node_representation, node_char_outputs], 2)
            passage_node_representation = tf.concat([passage_node_representation, passage_nodes_POS_representation], 2)
        else:
            input_dim = word_dim + POS_dim
            passage_node_representation = tf.concat([passage_node_representation, passage_nodes_POS_representation], 2)
        ...
        with tf.variable_scope('graph_encoder'):
            ...
            # [batch_size, passage_len, passage_neighbors_size_max, input_dim] collect neighbor node representations
            ...
            self.node_representations = passage_node_representation  #node representation + char representation
